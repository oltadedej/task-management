...
2. Replace `http://localhost:5000` with your backend URL if it's running on a different address.
...
The API base URL is configured in `src/services/api.service.ts` and can be overridden via environment variable `VITE_API_BASE_URL`.
...
| GET | `/api/v1/tasks/status/{status}` | Get tasks by status (0=NotStarted, 1=InProgress, 2=Completed) |
| POST | `/api/v1/tasks` | Create a new task |
| PUT | `/api/v1/tasks/{id}` | Update an existing task |
| DELETE | `/api/v1/tasks/{id}` | Delete a task |
| PATCH | `/api/v1/tasks/{id}/complete` | Mark task as complete |
| PATCH | `/api/v1/tasks/{id}/incomplete` | Mark task as incomplete (Not Started) |
...
2. Check the `VITE_API_BASE_URL` environment variable matches your backend URL
3. Verify CORS is properly configured in the backend (see backend README)
...
2. Review the backend API documentation (Swagger UI)
